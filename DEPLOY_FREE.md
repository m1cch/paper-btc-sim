...
**История сделок после деплоя:** на free **локальный SQLite в контейнере обнуляется** при новом деплое. Чтобы **всё сохранялось**, задай в Render переменную **`DATABASE_URL`** на внешний **PostgreSQL** (бесплатно: [Neon](https://neon.tech), [Supabase](https://supabase.com) и т.п.). Скопируй строку подключения `postgresql://...` в **Environment** веб-сервиса и сделай **Redeploy**. Без `DATABASE_URL` бот по-прежнему использует SQLite (удобно локально).
...
5. **Переменные** (уже в `render.yaml`, при необходимости правь в Dashboard):  
   `AUTO_TRADE`, `INITIAL_DEPOSIT`, `GAMMA_SERIES_ID`, и при необходимости **`DATABASE_URL`** (см. выше).
